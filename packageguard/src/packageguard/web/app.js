...
const KIND_CLASS = { clean: "chip-clean", typosquat: "chip-bad", malware: "chip-bad", demo: "chip-demo", dataset: "chip-dataset", "real-poison": "chip-realpoison" };
...
function fillChips(id, items, onClick, labelFn) {
  ...
  items.forEach((e) => {
    ...
    if (e.kind === "dataset") b.title = "Real benign package pulled live from the current trained dataset";
    if (e.kind === "real-poison") b.title = "A real npm package whose live dependency graph contains a genuine labeled-malicious package — not a curated demo";
...
// ---------- home page: live micro-scan widget (self-contained, own fetch, doesn't
// touch the CHECK tab's own state — only jumps + re-runs it on "FULL BREAKDOWN") ----------
(function () {
  const form = document.getElementById("qscan-form");
  if (!form) return;
  const input = document.getElementById("qscan-input");
  const resultEl = document.getElementById("qscan-result");
  ...
  form.addEventListener("submit", async (e) => {
    ...
    resultEl.className = "qscan-result loading";
    ...
    try {
      ...
      resultEl.className = "qscan-result";
      if (d.not_found) {
        resultEl.innerHTML = `<span class="qscan-verdict low">NOT FOUND</span><span class="qscan-score">— not a real npm package</span>`;
        return;
      }
      const level = (d.level || "low").toLowerCase();
      resultEl.innerHTML = `
        <span class="qscan-verdict ${level}">${d.verdict}</span>
        <span class="qscan-score">score ${d.score}</span>
        <button type="button" class="qscan-more" id="qscan-more">FULL BREAKDOWN ▸</button>`;
      document.getElementById("qscan-more").addEventListener("click", () => {
        const tab = document.querySelector('.tab[data-tab="check"]');
        if (tab) tab.click();
        const full = document.getElementById("pkg-input");
      ...
      resultEl.className = "qscan-result";
      resultEl.innerHTML = `<span class="qscan-verdict high">⚠ ${err.message}</span>`;
    }
  });
})();
